#
#  This is https://github.com/sizmailov/pybind11-stubgen v0.10.2
# 
#  Edits marked by STC
#  TODO: fork repo and add features formally
#
...
class AttributeStubsGenerator(StubsGenerator):
    ...
    def to_lines(self):  # type: () -> List[str]
        ...
        if len(value_lines) == 1:
            ...
            #STC: Added =None so that variable exists for sphinx to find
            return [
                "{name}: {typename} = None{value_comment}".format( 
                    name=self.name,
                    typename=typename,
...
class ModuleStubsGenerator(StubsGenerator):
    ...
    def to_lines(self):  # type: () -> List[str]
        ...
        #STC: Remove import of our own module - not sure what this does
        #result += [
    ...
    def write(self):
        with DirectoryWalkerGuard(self.short_name + self.stub_suffix):
            #STC: changed pyi -> py
            with open("__init__.py", "w", encoding="utf-8") as init_pyi: 
                init_pyi.write("\n".join(self.to_lines()))
